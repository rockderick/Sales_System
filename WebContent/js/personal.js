	...
	function updateBox(form,item){
		...
		changeInput(form.pcFchBaja,item.value,'fchbaja');
	...
	function goToAgregarAdmin(form, control) {
		...
		MM_validateForma('pcNomVendedor','RisText', 'Nombre vendedor:',
						...
						'pcTipoComision','RisCero','Tipo de Comision:',
...
function goToHorario(form) {
	...
	function goToSuperior(form) {
		...
		if(rad!=null){
			...
			var clave= radioValue(form.pcCveVendedor);
			validarEntero(clave);
			window.open("PersonalMovAction.do?action=buscarSuperior&pcCveVendedor="+clave+"",
			"BuscaSuperior","width=730,height=500,status=0, menubar=0,toolbar=0, " +
			"resizable=1, top=100, left=100, scrollbars=1");
			
			}else{
			alert("Seleccione el vendedor de la parte superior");
			}
			}else{
				alert("Busque un vendedor en la parte superior");
			}
	 }
	...
	function keyDownNombre(e, forma, control){
		var k =  e.keyCode; 
		
		if (k == 13) {
			goToSearch(forma, control);
		}	
	}