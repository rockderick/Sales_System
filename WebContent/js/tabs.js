...
function activateMenu() { 
    ...
    var found = 0;
    for (var i=0; i < items.length; i++) {
        var url = trimParamaters(items[i].firstChild.getAttribute("href"));
        var current = document.location.toString();
        if (current.indexOf(url) != -1) {
            found++;
        }
    }
    //checar si existe parentNode
		    ...
		    if (found > 1) {  
		        ...
		        for (var i=0; i < items.length; i++) {
		           ...
		           // alert("else > 1" )
		            ...
		            if(items[i].parentNode == undefined || items[i].parentNode !=null) {
...
function trimParamaters(url){
	var indice = url.indexOf('?');
	return url.substr(0,indice);
}
