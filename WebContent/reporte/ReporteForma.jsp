...
<%@ page import="mx.com.iusacell.catalogo.web.personal.struts.action.PersonalMovProxy"%>
...
String cveVendedor = "";
cveVendedor = (String)request.getAttribute("nombreVendedorBuscar");
...
<logic:present name="jsArray">
		...
		function goToExcel(form, control) {
			...
			if(form.pcCveDivisionSeek.value == "" && form.pcCveVendedorSeek.value == ""){
			          alert('Debes de especificar al menos un criterio de búsqueda.');
			         return false;
		   }
		   if(form.pcCveVendedorSeek!=null){
		        if(isNaN(form.pcCveVendedorSeek.value)){
		         alert('La clave de vendedor debe ser numérica, favor de revisar.');
		         return false;
		       }
		     }	
			...
			if(control == null || control==undefined){
			   if(form.pcCveDivisionSeek.value == "" && form.pcCveVendedorSeek.value == ""){
				...
				if(control=="excelHistorico") {
				   alert('hoooo');
					if(document.forms[0].pcFchHistorico.value =="") {
						alert("Debes de especificar una fecha para el reporte de Historico")
						return false;
					}
...
<%
       String user = divisionUsuario.getPcUserid();
       boolean encontrado = ((UserSessionVO) request.getSession().getAttribute("USER")).isPcValidadoConfig();
       int divisionDelUsuario = divisionUsuario.getPcCveDiv();
       int administrador = 0;
       if(encontrado || divisionDelUsuario==0)administrador = 1; 
%> 
      <TR>
...
<%}%>
		<TR BGCOLOR="#FFFFFF">
			...
			<TD WIDTH="40%" ALIGN="LEFT" CLASS="PTabData">
				<html:text property="pcCveVendedorSeek" name="ReporteArbolForm" maxlength="7" size="30" value="<%= cveVendedor %>" onchange="this.value=this.value.toUpperCase()"/>
			</TD>
		</TR>
<!--		<TR BGCOLOR="#F9F9F9">
			<TD WIDTH="10%"  CLASS="txtContenidoRojo"><bean:message key="reporteForm.pcNomClaveEmpleadoReferenciaSeek"/> :</TD>
			<TD WIDTH="40%" ALIGN="LEFT" CLASS="PTabData">
				<html:text property="pcNomClaveEmpleadoReferenciaSeek" name="ReporteArbolForm" maxlength="40" size="30" value="" onchange="this.value=this.value.toUpperCase()"/>
			</TD>
		</TR>
		<TR bgcolor="FFFFFF">
			<td WIDTH="10%" CLASS="txtContenidoRojo"><bean:message key="reporteForm.pcFchHistorico" /> <bean:message key="mascara.fecha"/>: </td>
			<td WIDTH="40%" CLASS="txtContenido"><html:text property="pcFchHistorico" name="ReporteArbolForm" maxlength="10" size="10" onchange="this.value=this.value.toUpperCase()"/>
			<a href="javascript:show_calendar('ReporteArbolForm.pcFchHistorico','','','DD/MM/YYYY','es')">
...
<logic:present name="noData">
		...
		<!--<TD WIDTH="40%" ALIGN="CENTER" CLASS="txtContenido">
 		...
 		<TD WIDTH="40%" ALIGN="CENTER" CLASS="txtContenido">
			<html:button property="boton" styleClass="boton" onclick="goToExcel(ReporteArbolForm,'excelHistorico')">Reporte Excel Historico</html:button>
 		</TD>--> 		
 		<TD>
...
<td><font size=-2><a style="font-size:7pt;text-decoration:none;color:#FFFFFF" href="http://www.treemenu.net/" target=_blank>JavaScript Tree Menu</a></font></td>
	...
	<TD valign="top" width="54%">
			...
			<tr bgcolor="#FFFFFF"><td class="txtContenido" width="25%"><bean:message key="reporteForm.divPcCveEmpRef.displayname"/></td><td class="txtContenido"><span ID="divPcFax">&nbsp;</span></td></tr>
