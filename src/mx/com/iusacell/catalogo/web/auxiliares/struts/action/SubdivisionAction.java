...
public class SubdivisionAction extends Action {
	...
	protected static final Logger logger = Logger.getLogger(SubdivisionAction.class);
	
	public SubdivisionAction() {
	}
	...
	private ActionForward home(ActionMapping mapping, ActionForm form,
		HttpServletRequest request, HttpServletResponse response) {
			return new  SubdivisionProxy().verLista(mapping,form,request,response);	
	}

	private ActionForward agregar(ActionMapping mapping, ActionForm form,
		HttpServletRequest request, HttpServletResponse response) {

			ActionMessages errors = (ActionMessages) ((SubdivisionForm) form).validate(mapping, request);
			if (errors != null && errors.isEmpty()) {
				return new  SubdivisionProxy().agregar(mapping,form,request,response);	
			} else {
	...
	private ActionForward eliminar(ActionMapping mapping, ActionForm form,
		HttpServletRequest request, HttpServletResponse response) {
			...
			return new  SubdivisionProxy().eliminar(mapping,form,request,response);	
	}
...
//			if (errors != null && errors.isEmpty()) {
				return new  SubdivisionProxy().modificar(mapping,form,request,response);	
//			} else {
//				saveErrors(request, errors);
//				return mapping.findForward("home");
//			}				
	...
	private ActionForward consultar(ActionMapping mapping, ActionForm form,
		HttpServletRequest request, HttpServletResponse response) {
			...
			if (errors != null && errors.isEmpty()) {
				return new  SubdivisionProxy().consultar(mapping,form,request,response);	
			} else {
				saveErrors(request, errors);
				return mapping.findForward("home");
			}				
