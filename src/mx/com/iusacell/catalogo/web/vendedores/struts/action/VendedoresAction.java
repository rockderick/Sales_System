 ...
 *
 */
public class VendedoresAction extends Action {

	protected static final Logger logger = Logger.getLogger(VendedoresAction.class);
	
	public VendedoresAction() {
	}
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
			...
			try {
			 ...
			 //   saveToken(request);
				...
				logger.equals("Error en execute:" + ex.toString());
			}
			//return null;
	...
	private ActionForward home(ActionMapping mapping, ActionForm form,
		HttpServletRequest request, HttpServletResponse response) {
			return new  VendedoresProxy().verLista(mapping,form,request,response);	
	}
			...
			try{
				...
				if(control.equals("siguiente")){
					...
					if (errors != null && errors.isEmpty()) {
						return new  VendedoresProxy().consultar(mapping,form,request,response);
					} else {
	...
	private ActionForward agregar(ActionMapping mapping, ActionForm form,
		HttpServletRequest request, HttpServletResponse response) {
			ActionMessages errors = (ActionMessages) ((VendedoresForm) form).validate(mapping, request);
			if (errors != null && errors.isEmpty()) {
				return new  VendedoresProxy().agregar(mapping,form,request,response);	
			} else {
				saveErrors(request, errors);
				return mapping.findForward("home");
			}
	...
	private ActionForward modificar(ActionMapping mapping, ActionForm form,
		HttpServletRequest request, HttpServletResponse response) {
			return new VendedoresProxy().modificar(mapping,form,request,response);	
	}

	private ActionForward eliminar(ActionMapping mapping, ActionForm form,
		HttpServletRequest request, HttpServletResponse response) {
			return new  VendedoresProxy().eliminar(mapping,form,request,response);	
	}
	...
	*/
	...
	private ActionForward horario(ActionMapping mapping, ActionForm form,
		HttpServletRequest request, HttpServletResponse response) {
			return new  VendedoresProxy().horario(mapping,form,request,response);	
	}	
}
