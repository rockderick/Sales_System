...
/**
 * <code>MessageFormatIds</code> provides a means to produce concatenated
 * messages in language-neutral way. Use this to construct messages
 * displayed for end users.
 *
 * <p>
 * <code>MessageFormatIds</code> takes a set of objects, formats them, then
 * inserts the formatted strings into the pattern at the appropriate places.
 *
 * <p>
 * <strong>Note:</strong>
 * <code>MessageFormatIds</code> differs from the other <code>Format</code>
 * classes in that you create a <code>MessageFormatIds</code> object with one
 * of its constructors (not with a <code>getInstance</code> style factory
 * method). The factory methods aren't necessary because <code>MessageFormatIds</code>
 * doesn't require any complex setup for a given locale. In fact,
 * <code>MessageFormatIds</code> doesn't implement any locale specific behavior
 ...
 * };
 *
 * String result = MessageFormatIds.format(
 ...
 *
 * MessageFormatIds form = new MessageFormatIds(
 ...
 * <blockquote>
 * <pre>
 * MessageFormatIds form = new MessageFormatIds("The disk \"{1}\" contains {0}.");
 ...
 * by a <code>ChoiceFormat</code> in <code>MessageFormatIds</code> is treated specially;
 * occurances of '{' are used to indicated subformats, and cause recursion.
 * If you create both a <code>MessageFormatIds</code> and <code>ChoiceFormat</code>
 ...
 * <pre>
 * MessageFormatIds mf = new MessageFormatIds("{0,number,#.##}, {0,number,#.#}");
 ...
 * </pre>
 * <p>
 * Likewise, parsing with a MessageFormatIds object using patterns containing
 * multiple occurances of the same argument would return the last match.  For
 * example,
 * <pre>
 * MessageFormatIds mf = new MessageFormatIds("{0}, {0}, {0}");
 ...
 * <code>MessageFormatIds</code> with a different locale.
 *
...
public class MessageFormatIDS extends Format {
	/**
	 * Constructs with the specified pattern.
	 * @see MessageFormatIds#applyPattern
	 */
	public MessageFormatIDS(String pattern) {
	...
	/**
	 * Convenience routine.
	 * Avoids explicit creation of MessageFormatIds,
	 * but doesn't allow future optimizations.
	 */
	public static String format(String pattern, Object[] arguments) {
			MessageFormatIDS temp = new MessageFormatIDS(pattern);
	...
	public Object[] parse(String source) throws ParseException {
		...
		if (status.getIndex() == 0)  // unchanged, returned object is null
			throw new ParseException("MessageFormatIds parse error!", status.getErrorIndex());
	...
	public Object clone()
	{
		MessageFormatIDS other = (MessageFormatIDS) super.clone();
	...
	public boolean equals(Object obj) {
		...
		if (obj == null || getClass() != obj.getClass())
			return false;
		MessageFormatIDS other = (MessageFormatIDS) obj;
		return (maxOffset == other.maxOffset
				&& pattern.equals(other.pattern)
			&& Utilidad.objectEquals(locale, other.locale)   // does null check
				&& Utilidad.arrayEquals(offsets,other.offsets)
			&& Utilidad.arrayEquals(argumentNumbers,other.argumentNumbers)
			&& Utilidad.arrayEquals(formats,other.formats));
	}
	...
	private static final int MAX_ARGUMENTS = 50;
	 ...
	 * @see MessageFormatIds#applyPattern
	 */
	private MessageFormatIDS(String pattern, Locale loc) {
		...
		for (int i = 0; i <= maxOffset; ++i) {
			...
			if (false) { // if (argRecursion == 3){
				...
				if (tryRecursion && arg.indexOf('{') >= 0) {
					MessageFormatIDS temp = new MessageFormatIDS(arg, locale);
	...
	private void readObject(ObjectInputStream in) throws IOException, ClassNotFoundException {
		...
		if (!isValid) {
			throw new InvalidObjectException("Could not reconstruct MessageFormatIds from corrupt stream.");
		}
	}
}