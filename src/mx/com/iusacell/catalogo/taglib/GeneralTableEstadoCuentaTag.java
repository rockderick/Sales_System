 ...
 * Window&gt;Preferences&gt;Java&gt;Code Generation&gt;Code and Comments
 */
public class GeneralTableEstadoCuentaTag extends TagSupport {
	...
	/** Script a agregar al campo resaltado**/
	...
	/**
	 * Nombre del metodo a ejecutar del VO para identificar la llave de la tabla
	 */
	...
	public void ajustaParametros(){
		...
		/*if(contador < numColumnas){
			throw new ComisionesSystemException("Faltan metodos para generar la tabla general", null);
		}*/
		...
		/*if(contador < numColumnas){
			throw new ComisionesSystemException("Faltan alineaciones para generar la tabla general", null);
		}*/
		...
		/*if(contador < numColumnas){
			throw new ComisionesSystemException("Faltan anchos para generar la tabla general", null);
		}*/
	}
	...
	protected void realizaConsulta() throws SQLException {
		...
		for (int i= 0; i<=3; i++){
			...
			estadocuenta.setPcDesc("Activaciones");
			estadocuenta.setPcNumMovim("13");
			estadocuenta.setPcTotMonto("10000");
			estadocuenta.setEstimadoUno("51-100");
			estadocuenta.setEstimadoDos("101-150");
			estadocuenta.setEstimadoTres("151-300");
			arreglo.add(estadocuenta);
		}
	...
	protected void generaTitulosTable() throws IOException{
		...
		for(int i = 0; i <= 3; i++){
			...
			if(i == 3){
				writer.print(Html.cadenaCeldaTituloEstimadaTable((String)anchosColumnasArray.get(i), cssColorEncabezado, altoRenglon, (String)alineacionesColumnasArray.get(i), (String)titulosArray.get(i)));
			}
			if(i != 3){
				writer.print(Html.cadenaCeldaTituloTable((String)anchosColumnasArray.get(i), cssColorEncabezado, altoRenglon, (String)alineacionesColumnasArray.get(i), (String)titulosArray.get(i)));
			}
		}
		doAfterTitulos();
	...
	protected void generaRenglonesTable() throws IOException{
		...
		JspWriter writer = super.pageContext.getOut();
		int numeroRenglones = data.size();
		int conta = 0;
	
			/*for(int i = 0; i < data.size(); i++){
				if(i != numeroRenglones-1) {
					
					generaRenglonTable(data.get(i), (i % 2 == 0 ? cssColorRenglon1 : cssColorRenglon2), i, script);
				} else {
					
					generaRenglonNormalTable(data.get(i), (i % 2 == 0 ? cssColorRenglon1 : cssColorRenglon2), i);	
	 ...
	 * @throws IOException
	 */
	protected void generaRenglonTable(Object elemento, String color, int numElemento, String funcionJavaScript) throws IOException{
		JspWriter writer = super.pageContext.getOut();
		writer.print(Html.cadenaHeaderRowTable());
		doBeforeCeldas(elemento, color, numElemento);
		int numeroColumnasTablaEstadoCuenta = numColumnas;
		numeroColumnasTablaEstadoCuenta = numeroColumnasTablaEstadoCuenta + 2;
		
		try{
			for(int i = 0; i < numeroColumnasTablaEstadoCuenta; i++){
				generaCeldaTable(elemento, color, numElemento, i, funcionJavaScript);
			}
	 ...
	 * @throws IOException
	 */
	protected void generaRenglonNormalTable(Object elemento, String color, int numElemento) throws IOException{
		JspWriter writer = super.pageContext.getOut();
		writer.print(Html.cadenaHeaderRowTable());
		doBeforeCeldas(elemento, color, numElemento);
		int numeroColumnasTablaEstadoCuenta = numColumnas;
		...
		try{
			for(int i = 0; i < numeroColumnasTablaEstadoCuenta; i++){
				generaCeldaNormalTable(elemento, color, numElemento, i);
			}
		} catch (NoSuchMethodException e) {
			throw new CatalogoSystemException("Error al generar la tabla general ", e);
		} catch (IllegalAccessException e) {
	 ...
	 */
	protected void generaCeldaTable(Object elemento, String color, int numRenglon, int numColumna, String funcionJavaScript) throws NoSuchMethodException, IllegalAccessException, InvocationTargetException, IOException{
		JspWriter writer = super.pageContext.getOut();
		if( numColumna == 0){
			writer.print(Html.cadenaCeldaResaltadaEstadoCuentaTable((String)anchosColumnasArray.get(numColumna), color, altoRenglon, (String)alineacionesColumnasArray.get(numColumna), convierteString(MethodUtils.invokeExactMethod(elemento, (String)metodosArray.get(numColumna), null), (String)metodosArray.get(numColumna)),funcionJavaScript,grupo,convierteString(MethodUtils.invokeExactMethod(elemento, (String)metodosArray.get(6), null), (String)metodosArray.get(6)),convierteString(MethodUtils.invokeExactMethod(elemento, (String)metodosArray.get(7), null), (String)metodosArray.get(7))));
		}else {
			writer.print(Html.cadenaCeldaTableEstadoCuenta((String)anchosColumnasArray.get(numColumna), color, altoRenglon, (String)alineacionesColumnasArray.get(numColumna), convierteString(MethodUtils.invokeExactMethod(elemento, (String)metodosArray.get(numColumna), null), (String)metodosArray.get(numColumna)),convierteString(MethodUtils.invokeExactMethod(elemento, (String)metodosArray.get(6), null), (String)metodosArray.get(6)),convierteString(MethodUtils.invokeExactMethod(elemento, (String)metodosArray.get(7), null), (String)metodosArray.get(7)),numColumna));
		}
	}
	 ...
	 * @throws InvocationTargetException
	 */
	protected void generaCeldaNormalTable(Object elemento, String color, int numRenglon, int numColumna) throws NoSuchMethodException, IllegalAccessException, InvocationTargetException, IOException{
		JspWriter writer = super.pageContext.getOut();
	 ...
	 * @param string
	 */
	public void setLlave(String string) {
		llave = string;
	}
